function x_0_barrier = interior_point_method_phase1(s)
    ...
    x_initial = pinv(A) * b;
    find_s_initial = s.find_s_initial;
    find_s_initial = @(x) find_s_initial(x);
    s_initial = find_s_initial(x_initial);
    x_k_nm = x_initial;
    x_k_nm(end+1) = s_initial;
    
    % To be filled with all x_k_nm through the iterations of the Newton algorithm
    x_nm_all = [x_k_nm]; %#ok<NBRAK>
    ...
    while(1)
        ...
        while(1)
            ...
            % Check feasibility
            while (~is_feasible(x_k_nm(1:end-1) + t_nm*delta_x_k_nm(1:end-1)))
            ...
            x_nm_all = [x_nm_all x_k_nm]; %#ok<AGROW>
