function x_0_barrier = findFeasibleBarrier(A,b,n,p,m,e,C_hat)
    ...
    phi = @(x) phi_nbm_phase1(x,C_hat);
    grad_phi = @(x) grad_phi_nbm_phase1(x,C_hat);
    hessian_phi = @(x) hessian_phi_nbm_phase1(x,C_hat);
    ...
    x_k_nm = pinv(A) * b;
    x_k_nm(end+1) = max([-x_k_nm ; -C_hat*x_k_nm]) + 1;
...
%                     (sum( - C_hat*x_test(1:end-1) - x_test(end) <= 0) ~= length(x_test) - 1))
%                 % Updated value of step scaler, t_nm, of Newton algorithm
%                 t_nm = beta*t_nm;
%                 x_test = x_k_nm + t_nm*delta_x_k_nm;
%             end
